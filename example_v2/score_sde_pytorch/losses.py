    ...
    def loss_fn(model, batch, labels=None):
        ...
        if "multimodel" in model.module.__class__.__name__:
            if biased:
                ...
                t_rand = [random.uniform(*random.choices(chose_interval, weights=[r[1]-r[0]
                                         for r in chose_interval])[0]) for i in range(batch.shape[0])]
            ...
            if fewer == 4:
                t = torch.rand(
                    batch.shape[0], device=batch.device) * (t2-t1-eps) + eps+t1
            else:
                t = torch.rand(
